...
app_ui(html::context ctx)
{
    ...
    placeholder_root(ctx, "app", [&] {
        alia::html::bootstrap::dropdown_button(ctx, "btn-primary", "Dzień Tygodnia", [&](auto& menu) {
        ...
        html::p(ctx, "Minute");
        html::input(ctx, new_minute);
        html::button(ctx, "Add lesson", add_lesson << new_key.read() << new_name.read() << new_hour.read() << new_minute.read() << new_weekday.read());
    });
}
